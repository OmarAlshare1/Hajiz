...
import CustomSelect from '../../components/CustomSelect'; // Import the new CustomSelect component
...
export default function ProvidersListPage() {
  ...
  const { data: allProviders, isLoading, error } = useQuery({
    queryKey: ['all-providers', searchQuery, selectedCategory, minRating], // Query key changes with filters
    queryFn: async () => {
      const params: { query?: string; category?: string; rating?: number } = {};
      if (searchQuery) {
        params.query = searchQuery; // Use 'query' param for text search
      }
      ...
      const res = await providers.getAll(params); // Pass params to the API call
      return res.data?.providers || res.data; // Ensure data structure matches API response
    },
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // The useQuery hook will automatically refetch due to queryKey change
  };
